...
struct alignas(64) RingBuffer {
  ...
  RingBuffer() { std::memset(data, 0, sizeof(data)); }
...
struct ThreadStorage {
  ...
  uint8_t stack_modes[MAX_ACTIVE_SLOTS];
  size_t stack_ptr = 0;
  std::map<ID, RingBuffer> history;
  __attribute__((always_inline)) inline RingBuffer* GetOrAdd(ID id) { return &history[id]; }
};
...
struct Recorder {
  ...
  __attribute__((always_inline)) static inline Cycles Stop(ID /*id*/) {
    ...
    if (__builtin_expect(ts->stack_ptr > 0, 1)) {
      ts->stack_ptr--;
      Cycles delta = end - ts->stack_starts[ts->stack_ptr];
...
inline void Manager::Calibrate() {
  ...
  constexpr int WARMUP_ITERS = 10000;
  ...
  auto BUMED = [&](ID id) -> Cycles {
    ...
    if (mins.empty()) return *std::min_element(raw.begin(), raw.end());
  ...
  for (size_t i = 0; i < Internal::CALIB_KEY_COUNT; ++i) calib_valid[i] = true;
...
inline void DumpToStream(std::ostream& oss, Parameter::Unit unit = Parameter::Cycle, Parameter::Data data_mode = Parameter::Raw) {
  ...
  if (unit == Parameter::Time || data_mode == Parameter::Calibrated) mgr.EnsureCalibrated();
  ...
  const int C1=20, C2=9, C3=10, C4=10, C5=10, C6=8, C7=10, C8=10, C9=10, C_BY=10;
  const int COL_COUNT=10;
  const int TABLE_WIDTH = C1+C2+C3+C4+C5+C6+C7+C8+C9+C_BY+(3*(COL_COUNT-1)+2);
  const std::string line(TABLE_WIDTH, '-');
  const std::string d_line(TABLE_WIDTH, '=');

  const char* LIGHT_GRAY = "\033[90m";
  const char* COLOR_RESET = "\033[0m";
  auto gray = [&](const std::string& s) { return std::string(LIGHT_GRAY) + s + COLOR_RESET; };
  ...
  if (data_mode == Parameter::Calibrated) {
    auto off_str = [&](uint8_t sm, uint8_t em) -> std::string {
      return ToDisp((double)mgr.CalibrationOffset(Internal::CalibKey(sm, em)));
    };
    ...
    const auto F=(uint8_t)Mode::Fast, M=(uint8_t)Mode::Mid, H=(uint8_t)Mode::Hard;
    ...
    write_row({mcol("H", 10, true)+mcol(off_str(H,F), MW)+mcol(off_str(H,M), MW)+mcol(off_str(H,H), MW)+END});
    write_row({mcol("PULSE", 10, true)+mcol(off_pulse_str(), MW)+mcol("", MW)+mcol("", MW)+END});
    oss << gray("|") << gray(line) << gray("|") << "\n";
  }

  write_row({col("COMPONENT",C1,true),col("SAMPLES",C2),col("AVG",C3),col("MEDIAN",C4),col("STD DEV",C5),col("SKEW",C6),col("MIN",C7),col("MAX",C8),col("RANGE",C9),col("OUTLIER",C_BY)});
  oss << gray("|") << gray(line) << gray("|") << "\n";
  ...
  for (auto& [id, series] : global_data) {
    ...
    for (double v : series.values) { double x = v - off; adjusted.push_back(x < 0.0 ? 0.0 : x); }
    ...
    for (double v : clean_values) { double d = v - avg; var_sum += d*d; skew_sum += d*d*d; }
    ...
    write_row({col(component_name,C1,true),col(std::to_string(n),C2),col(ToDisp(avg),C3),col(ToDisp(median),C4),col(ToDisp(std_dev),C5),col(sk.str(),C6),col(ToDisp(clean_values.front()),C7),col(ToDisp(clean_values.back()),C8),col(ToDisp(clean_values.back()-clean_values.front()),C9),col(std::to_string(outlier_count),C_BY)});
  }
...
namespace Latte {
  ...
  inline void DumpToStream(std::ostream&,
