...
// CONFIG
// ============================================================
static constexpr int SIM_STEPS =
    5'000;  // simulation horizon in ticks (reduce for fast compute)
static constexpr double DT_SECONDS = 0.02;  // 20ms decision loop
static constexpr double TICK_SIZE = 0.01;   // price grid
static constexpr double HALF_SPREAD =
    TICK_SIZE * 0.5;  // one-tick spread assumption
...
static constexpr double LAMBDA_IMB =
    0.35;  // imbalance sensitivity to signal in [-1,1]
...
static constexpr double MU0 =
    0.002;  // drift amplitude (price units / second) when z=+1
...
static inline int y_to_idx(int y) { return clamp_y(y) + Y_MAX; }
...
static inline int pro_rata_fill(int M, int Q, int D_others) {
  if (Q <= 0) return 0;
  const double share =
      static_cast<double>(Q) / static_cast<double>(Q + D_others);
...
class ProRataMM {
 public:
  explicit ProRataMM(const Params& p)
      : P(p),
        rng(42),
        N(SIM_STEPS),
        V((N + 1) * Y_SIZE * Z_STATES, -1e100),
        Pi(N * Y_SIZE * Z_STATES) {
    LATTE_RAII(Hard);  // Init_Precompute (auto-named "ProRataMM")
    m_pmf = build_m_pmf();
  ...
  void BuildPolicyDP() {
    ...
    for (int qb = 0; qb < Q_LEVELS; ++qb)
      for (int qa = 0; qa < Q_LEVELS; ++qa) make_actions.push_back({qb, qa});


    for (int t = N - 1; t >= 0; --t) {  // Backward
      LATTE_RAII();  // DP_TimeSlice (auto-named "BuildPolicyDP")
      ...
      for (int zi = 0; zi < Z_STATES; ++zi) {
        ...
        double lambda_ask =
            LAMBDA_BASE * (1.0 + LAMBDA_IMB * static_cast<double>(z));
        double lambda_bid =
            LAMBDA_BASE * (1.0 - LAMBDA_IMB * static_cast<double>(z));
        ...
        for (int y = -Y_MAX; y <= Y_MAX; ++y) {
          ...
          for (auto imp : impulses) {
            ...
            int dy = 0;
            if (imp == Impulse::FLATTEN)
              dy = -y;
            else if (imp == Impulse::BUY1)
              dy = +1;
            else if (imp == Impulse::SELL1)
              dy = -1;
            else if (imp == Impulse::BUY2)
              dy = +2;
            else if (imp == Impulse::SELL2)
              dy = -2;
            ...
            for (const auto& ma : make_actions) {
              ...
              for (int zj = 0; zj < Z_STATES; ++zj) {
                ...
                for (int vb = 0; vb <= V_MAX; ++vb) {
                  ...
                  for (int va = 0; va <= V_MAX; ++va) {
                    ...
                    const double make_gain =
                        P.half_spread * static_cast<double>(vb + va);
              ...
              if (val > best) {
                best = val;
                bestA = Action{
                    static_cast<int8_t>(qb), static_cast<int8_t>(qa), imp
                };
              }
            }
          }
  ...
  void RunSimulation() {
    LATTE_RAII(Hard);  // Sim_Total (auto-named "RunSimulation")
    ...
    for (int t = 0; t < N; ++t) {
      ...
      int dy = 0;
      if (a.imp == Impulse::FLATTEN)
        dy = -y;
      else if (a.imp == Impulse::BUY1)
        dy = +1;
      else if (a.imp == Impulse::SELL1)
        dy = -1;
      else if (a.imp == Impulse::BUY2)
        dy = +2;
      else if (a.imp == Impulse::SELL2)
        dy = -2;
      ...
      double lambda_ask =
          LAMBDA_BASE *
          (1.0 +
           LAMBDA_IMB * static_cast<double>(z));  // buys hit ask -> we sell
      double lambda_bid =
          LAMBDA_BASE *
          (1.0 -
           LAMBDA_IMB * static_cast<double>(z));  // sells hit bid -> we buy
      lambda_ask = std::max(1e-6, lambda_ask);
      lambda_bid = std::max(1e-6, lambda_bid);
    ...
    if (y != 0) {
      const double px =
          (y > 0)
              ? (S - HALF_SPREAD)
              : (S + HALF_SPREAD);  // sell long at bid, buy back short at ask
    ...
    std::cout << "Final MTM (liquidated): " << std::fixed
              << std::setprecision(6) << final_mtm << "\n";
  }
...
int main() {
  ...
  std::cout << "SIM_STEPS=" << SIM_STEPS << "  DT=" << DT_SECONDS
            << "s  Y_MAX=" << Y_MAX << "\n";
